...
type SessionMessage =
  | { type?: 'ready' }
  | { type?: 'refresh' }
  | { type?: 'cancel'; pid?: number }
  | { type?: 'terminate'; pid?: number };

export class SessionMonitorPanel {
  static async open(
    ...
    panel.webview.onDidReceiveMessage(async (message: SessionMessage) => {
  ...
  private static async postState(
  ...
  private static html(webview: vscode.Webview, connection: ConnectionConfig): string {
