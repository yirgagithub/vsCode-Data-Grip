...
export function StatusBar({ tab, resultSet }: { tab: QueryResultTab; resultSet?: ResultSet }) {
  return (
    <footer className="statusbar">
      ...
      <span className="statusbar-item">{(resultSet?.rowCount ?? tab.rowCount ?? 0).toLocaleString()} rows</span>
      <span className="statusbar-item">{tab.maxRows ? `limit ${tab.maxRows.toLocaleString()}` : 'all rows'}</span>
      <span className="statusbar-item">execution: {(tab.executionTimeMs ?? resultSet?.durationMs ?? 0).toLocaleString()} ms</span>
