...
export interface ConnectionEngineGuidance {
  hostLabel: string;
  ...
  authHelp: string;
  sslHelp: string;
  defaultSchemaHelp: string;
  required: {
    host: boolean;
    username: boolean;
    database: boolean;
  };
  disabled?: Partial<Record<'host' | 'port' | 'username' | 'password' | 'sslMode', boolean>>;
}

export const ENGINE_GUIDANCE_BY_DATABASE_TYPE: Record<DatabaseType, ConnectionEngineGuidance> = {
  postgres: {
    hostLabel: 'Host:',
