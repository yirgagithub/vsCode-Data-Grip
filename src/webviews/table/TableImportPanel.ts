...
import { TableImportColumnMapping, TableImportPreview } from '../../services/tableImportService';
import { qualifiedName } from '../../utils/identifiers';

type ImportPanelMessage =
  | { type?: 'import'; mapping?: Array<Pick<TableImportColumnMapping, 'source' | 'target'>> }
  | { type?: 'cancel' };

export interface TableImportPanelRequest {
...
export class TableImportPanel {
  static async open(
    context: vscode.ExtensionContext,
    request: TableImportPanelRequest,
    onImport: (mapping: Array<Pick<TableImportColumnMapping, 'source' | 'target'>>) => Promise<TableImportResult>
  ): Promise<void> {
  ...
  private static html(webview: vscode.Webview, extensionUri: vscode.Uri, request: TableImportPanelRequest): string {
