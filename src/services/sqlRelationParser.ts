export interface ParsedSqlAlias {
  alias: string;
  schema?: string;
  table: string;
...
export function extractSqlAliases(sql: string): ParsedSqlAlias[] {
  const aliases: ParsedSqlAlias[] = [];
  let depth = 0;
  let single = false;
  let double = false;
...
function readRelationAlias(sql: string, start: number): { alias: ParsedSqlAlias; end: number } | undefined {
...
function readQualifiedIdentifier(sql: string, start: number): { schema?: string; table: string; end: number } | undefined {
...
function readIdentifier(sql: string, start: number): { value: string; end: number } | undefined {
  if (sql[start] === '"') {
    let value = '';
...
function readIdentifierWord(sql: string, start: number): { value: string; end: number } | undefined {
