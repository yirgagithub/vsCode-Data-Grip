...
import {
  ...
  TablePerformancePrepassFlag,
  TablePerformanceRecommendation,
  TableStatsInfo,
...
export interface TablePerformanceAdvisorReport {
  request: TablePerformanceAdviceRequest;
  advice: TablePerformanceAdvice;
  aiError?: string;
}

export class TablePerformanceAdvisorService {
  constructor(
    private readonly connectionManager: ConnectionManager,
    private readonly memory: QueryMemoryService,
    private readonly ai: TablePerformanceAdvisorAi
  ) {}

  async analyzeTable(connection: ConnectionConfig, schema: string, table: string): Promise<TablePerformanceAdvisorReport> {
    ...
    const request: TablePerformanceAdviceRequest = {
...
function deterministicAdvice(flags: TablePerformancePrepassFlag[]): TablePerformanceAdvice {
  return {
...
function mergeDeterministicRecommendations(advice: TablePerformanceAdvice, flags: TablePerformancePrepassFlag[]): TablePerformanceAdvice {
...
function deterministicRecommendations(flags: TablePerformancePrepassFlag[]): TablePerformanceRecommendation[] {
  return flags
...
function topWorkloadColumn(workload: TableWorkloadSummary, role: TableWorkloadColumnRole): string | undefined {
...
function workloadEvidence(workload: TableWorkloadSummary, column: string): string {
