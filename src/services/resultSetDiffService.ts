...
export interface ResultSetDiffColumnChange {
  column: string;
  leftValue: unknown;
  rightValue: unknown;
}

export interface ResultSetDiffRowChange {
  key: string;
  leftRow: Record<string, unknown>;
  rightRow: Record<string, unknown>;
  changes: ResultSetDiffColumnChange[];
}

export interface ResultSetDiffRowDelta {
  key: string;
  row: Record<string, unknown>;
}

export interface ResultSetDiffReport {
  leftTitle: string;
  rightTitle: string;
  leftRowCount: number;
  ...
  addedRows: ResultSetDiffRowDelta[];
  removedRows: ResultSetDiffRowDelta[];
  changedRows: ResultSetDiffRowChange[];
  sameRows: number;
}

export function compareResultSets(left: ResultSet, right: ResultSet, leftTitle = left.title, rightTitle = right.title): ResultSetDiffReport {
  ...
  const addedRows: ResultSetDiffRowDelta[] = [];
  const removedRows: ResultSetDiffRowDelta[] = [];
  const changedRows: ResultSetDiffRowChange[] = [];
  let sameRows = 0;
...
export function formatResultSetDiffMarkdown(report: ResultSetDiffReport): string {
  const lines: string[] = [
    '# Result Set Diff',
    '',
...
function compareRows(left: Record<string, unknown>, right: Record<string, unknown>, columns: string[]): ResultSetDiffColumnChange[] {
  const changes: ResultSetDiffColumnChange[] = [];
...
function groupRows(
  rows: Array<{ row: Record<string, unknown>; index: number }>,
  identityColumns: string[],
  comparisonColumns: string[]
): Map<string, Array<{ row: Record<string, unknown>; index: number }>> {
...
function pickIdentityColumns(columns: string[]): string[] {
...
function rowKey(row: Record<string, unknown>, columns: string[], index: number): string {
  if (!columns.length) {
