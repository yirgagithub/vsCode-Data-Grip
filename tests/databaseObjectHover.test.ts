...
import { escapeMarkdownText, renderDatabaseObjectHover } from '../src/services/databaseObjectHover';
...
describe('renderDatabaseObjectHover', () => {
  it('renders compact table columns, nullability, primary and foreign keys', () => {
    ...
    expect(markdown).toContain('`team\\_id` — `integer` — NULL — FK → `public.teams\\.id`');
  ...
  it('renders views, functions, procedures, and triggers', () => {
    ...
    ] })).toContain('**View** `public.active\\_users`');
  ...
  it('escapes Markdown data and never creates links', () => {
    ...
    expect(markdown).not.toMatch(/\]\(|command:/);
  });
});
